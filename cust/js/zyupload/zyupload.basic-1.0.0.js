...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="cust/js/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type, para.fileType) > -1){
							// 类型符合，可以上传
							if ( para.fileSize != -1 && file.size >= para.fileSize) {
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "cust/js/zyupload/skins/images/fileType/";
				if(type == "rar"){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		</div>';
					html += '	</a>';
					html += '   <div class="upload_oper_box">';
					html += '   	<div class="file_zy_input">';
					html += '   		<input type="hidden" id="file_ids_'+file.index+'" name="file_ids[]"  width="142" >';
					html += '   		<input type="text" id="file_title_'+file.index+'" name="file_title[]" width="142" dataType="*">';
					html += '   	</div>';
					html += '   </div>';
