...
function initAudioEvent() {
    ...
    $.each(audios,function () {
        var audio = this;
        var index = $(audio).data("info");
        var audioPlayer = document.getElementById("audioPlayer"+index);
        var currentTime = $(audio).data('currenttime');
        var duration = $(audio).data('duration');
        audio.currentTime = currentTime;
        audio.duration = duration;
        updateProgress1(audio,index);
        // 监听音频播放时间并更新进度条
        ...
        audioPlayer.addEventListener('click', function () {
            ...
            if (audio.paused) {
                // 开始播放当前点击的音频
                audio.play();
                audioPlayer.src = 'cust/js/H5-audio/image/pause.png';
            } else {
                audio.pause();
...
function updateProgress1(audio,index) {
    var currentTime = $(audio).data("currenttime");
    var duration = $(audio).data("duration");
    var value = currentTime / duration;
    document.getElementById('progressBar'+index).style.width = value * 100 + '%';
    document.getElementById('progressDot'+index).style.left = value * 100 + '%';
    document.getElementById('audioCurTime'+index).innerText = transTime(currentTime);
}
/**
 * 播放完成时把进度调回开始的位置
 */
function audioEnded(index) {
    ...
    document.getElementById('audioPlayer'+index).src = 'cust/js/H5-audio/image/play.png';
}
