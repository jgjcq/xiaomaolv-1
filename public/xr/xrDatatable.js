...
// (function($) {
	$.fn.XRDataTable = function(options) {
		...
		var defaults = {
			...
			preFunc: function(){},
			//搜索按钮点击后，search之前
			afterSearchClick: function(){},
			//重置按钮提前操作
			resetBrfore: function() {},
			//当前所有数据
			datas: {}
		...
		return this.each(function() {
			...
			//重新制定ajaxurl
			$.fn.resetAjaxUrl = function(new_url) {
                opts.ajaxUrl=new_url;
            }
...
//			dom.before(retButtonDiv);
	...
	var toPage = function(dom, opts) {
		...
		$('.xrTableRow').hide(); 
		$('.noXrTableRow').remove();
		$('.loadXrTableRow').remove();
		$('.listcontent').append('<div class="row loadXrTableRow text-center"><div><i class="icon-spinner icon-spin"></i></div><div>数据加载中...</div></div>');
		opts.preFunc();
		$.ajax({
			url: opts.ajaxUrl,
			type: "post",
	...
	var viewOperateColumn = function(opts, trData) {
		...
		for(var key in opts.operation) {
			...
			if(!isFunction(opts.operation[key][0])) {
				...
				if(!isNewblank){
					var zdialog_url=translateWords(opts.operation[key][0], trData);
