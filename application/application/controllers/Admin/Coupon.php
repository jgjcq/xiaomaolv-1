...
class Coupon extends AdminController
{
    ...
    function getDatas($params = array(), $isExport = false) {
        ...
        foreach ($data["ret"] as $k => $v) {
            ...
            $data['ret'][$k]['zk_type_str']=$this->coupon_zk_type[$v['zk_type']];
        }
    ...
    function save(){
        $count = $this -> Coupon_model -> get_count(array("title"=>$_POST['title']));
        $goods = array();
        $userId = $_SESSION[SESS_USER]["id"];
        if(!isset($_POST['id']) || $_POST['id'] == ""){
            $goods = copyArray($_POST,array("title","type","zk","remark","zk_type","start_time","end_time"));
        }else{
            $goods = copyArray($_POST,array("id","title","remark","zk_type","start_time","end_time"));
        }

        if(!isset($_POST['id']) || $_POST['id'] == ""){
            if($count)
            {
                exit(retJson("该优惠卷已存在.", false));
            }
