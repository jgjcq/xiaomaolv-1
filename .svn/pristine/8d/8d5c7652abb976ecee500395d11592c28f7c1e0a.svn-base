...
(function($) {
	...
	var generateSearchAndRetButtons = function(dom,opts) {
			...
			retButtonDiv.addClass("col-xs-1 resetButton srBtn");
			retButtonDiv.css("float","right");
			var retButton = $("<a></a>");
			retButton.addClass("btn btn-default btn-sm")
			retButton.html("重置");
			retButtonDiv.append(retButton);
			$("#searchForm").append(retButtonDiv);
...
//			dom.before(searchButtonDiv);
				...
				for(var i = 0; i < ret.length; i++) {
					...
					for(var j = 0; j < tdNames.length; j++) {
						...
						if(tdNames[j] == 'checkbox') {
							td.html("<input type='checkbox' class='checkOne'/>");
						} else if(tdNames[j] == "xrno") {
							td.html((opts.pageNo - 1) * opts.pageSize + i + 1);
		...
		for(var key in opts.operation) {
			...
			if(!isFunction(opts.operation[key][0])) {
				...
				if(!isNewblank){
					...
					icon.attr("onclick", showZModal(885,520,'asdas',translateWords(opts.operation[key][0], trData)));
				}else{
	...
	var bindIconClick = function(ele,func){
		ele.click(function() {
					...
					var realIndex = selectedIndex - XRappendNum;
